  ...
  it("actually completes commands and option values under bash", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  COMPREPLY=()",
        "  _claude_cli_completions",
        '  echo "${COMPREPLY[*]}"',
        "}",
        'try claude-cli ""',
        "try claude-cli md gr",
        'try claude-cli md graph --output ""',
        'try claude-cli md audit --format ""',
        'try claude-cli md graph --format ""',
        'try claude-cli agent convert --target ""',
        'try claude-cli completion ""',
      ].join("\n"),
    );
