...
must never do. This is where it differs from [`scripts which`](scripts-which.md), which fails
...
[`scripts run`](scripts-run.md) refuses.
...
- [`scripts which`](scripts-which.md) explains one name in detail, including every file consulted.
- [`scripts run`](scripts-run.md) executes a listed name.
