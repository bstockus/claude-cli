...
| --------------------- | ------- | ------------------------------------------------------------------------------------------- |
| `--target <target>`   | None    | Repeatable target: `claude-code`, `codex`, `cursor`, or `all`. Enables the rendered checks. |
...
rendered output too, the same way [`agent package`](agent-package.md) does — a review can
...
`--baseline` takes the `sbom.json` [`agent package`](agent-package.md) writes. Scope is
...
`--strict` says so. Notices never block. A freshly scaffolded [`agent init`](agent-init.md)
